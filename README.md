...
Vespra is a self hosted AI agent system that grows your crypto for you. You give it a plain English goal, like *"Earn yield on 0.05 ETH on Base, WETH and USDC only"*, and a swarm of small AI agents handles the rest: finding opportunities, checking risk, executing swaps, monitoring your position, and rotating into better yields when they appear.
...
4. **Executes the swap** through a real DEX, whether Uniswap V3, Aerodrome, or whatever's wired up for that chain.
...
Each agent is just a focused LLM prompt plus some tool calls. They don't run continuously. They're invoked as the goal moves through its steps.
...
A small Zig service that runs DAG style workflows (steps with dependencies, retries, branching, and fan out) backed by its own SQLite database, with its API on port `9090`.

The gateway registers its nine agents with nullboiler as workers (one per agent tag) and can dispatch multistep **analysis** pipelines through it: a named workflow like `yield_rotation` expands into scout, then risk, then planning, where each step's output feeds the next via `{{steps.<id>.output}}` templating. These pipelines produce plans and assessments. The core goal pipeline that actually moves funds runs inside the gateway itself, so nullboiler stays a read only orchestration layer. Set `VESPRA_WORKER_CALLBACK_URL` to turn the integration on (it's wired for you in `docker-compose.yml`).

The named workflows live in the gateway's DAG registry, so adding a new pipeline is a matter of listing its steps there.
...
9. Keymaster wraps ETH into WETH if needed, approves the router, sends the actual `exactInputSingle` swap, and returns a tx hash.
...
* **`compound`**: accumulate gains and reenter with the bigger pot. *"Compound my ETH"*
...
* Docker plus Docker Compose (recommended) **or** the Rust toolchain plus Zig if you want to run from source
...
#   VESPRA_MASTER_PASSWORD=something_long_and_random   (>=16 chars)
...
| `VESPRA_GATEWAY_TOKEN` | bearer token for the gateway's own REST API; empty = open | unset |
| `VESPRA_KM_REQUIRE_MIN_OUT` | keymaster rejects swaps with no slippage floor | `false` |

RPC URLs follow the pattern `RPC_URL_{CHAIN}`: the suffix is lowercased and used as the chain key. So `RPC_URL_BASE_SEPOLIA` populates the `base_sepolia` chain, `RPC_URL_ARBITRUM` populates `arbitrum`, and so on.

## Safety

Vespra is built so that nothing can lose you more money than you've explicitly authorized. The layers, in order:

1. **Wallet caps**. Every wallet has a `cap_eth` (lifetime spend ceiling). Keymaster refuses to sign if a transaction would push the wallet over its cap, counting native sends *and* ETH/WETH sourced swaps. Set this to whatever you're comfortable losing.
...
3. **Risk gate**. Every position passes through the risk agent before execution. HIGH risk = no trade. (Note: on testnets, meaning any chain whose name contains `sepolia`, `testnet`, or `goerli`, the gate is deliberately relaxed to allow MEDIUM/HIGH so you can exercise the pipeline. Each relaxation is logged at `warn`. Do not run untrusted goals on a testnet chain name against mainnet funds.)
4. **Stop loss**. The sentinel exits any position that drops past your stop loss percentage, no questions asked.
5. **Kill switch**. The dashboard has a big red button that sets a global flag. While the flag is on, the orchestrators stop and the goal pipeline halts before every step, so no agent will scout, execute, or broadcast. Flip it via the API with `POST /swarm/kill`, and clear it with `POST /swarm/resume`.
6. **Dry run mode**. With `VESPRA_AUTO_EXECUTE_ENABLED=false` (the default) the gateway runs every step including building the calldata, but will *not* broadcast. You must set it to `true` **and** leave `VESPRA_DRY_RUN` unset/false to send real transactions. Safe by default.
7. **Slippage floor**. Swaps carry an on chain `amountOutMinimum` computed from a pretrade quote and `trader_max_slippage_pct` (default 1%), so a swap can't be sandwiched to zero. Set `VESPRA_KM_REQUIRE_MIN_OUT=true` on keymaster to *reject* any swap that arrives without a slippage floor (fail closed).
8. **Gateway auth**. Set `VESPRA_GATEWAY_TOKEN` to require a bearer token on every gateway request except `/health`. This is what stops someone who reaches `:9001` from driving the keymaster proxy. The bundled dashboard's nginx injects it automatically. Leave empty to run open behind the loopback bind / Cloudflare Access.
...
* `gateway-rs/src/agents/`, one file per agent, each a focused LLM wrapper
